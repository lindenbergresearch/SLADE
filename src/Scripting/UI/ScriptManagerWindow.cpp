...
public:
    ...
    ScriptManager::ScriptType selectedType() {
        switch (choice_type_->GetCurrentSelection()) {
            case 1:
                return ScriptManager::ScriptType::Archive;
            case 2:
                return ScriptManager::ScriptType::Entry;
            case 3:
                return ScriptManager::ScriptType::Map;
            default:
                return ScriptManager::ScriptType::Custom;
        }
    }
...
void ScriptManagerWindow::bindEvents() {
    // Tree item activate
    tree_scripts_->Bind(
        wxEVT_TREE_ITEM_ACTIVATED, [=](wxTreeEvent e) {
    ...
    tree_scripts_->Bind(
        wxEVT_TREE_ITEM_RIGHT_CLICK, [&](wxTreeEvent &e) {
            auto data = (ScriptTreeItemData *) tree_scripts_->GetItemData(e.GetItem());
    ...
    // Window close
    Bind(
        wxEVT_CLOSE_WINDOW, [=](wxCloseEvent e) {
            // Save Layout
            saveLayout();
    ...
    // Tab closing
    tabs_scripts_->Bind(
        wxEVT_AUINOTEBOOK_PAGE_CLOSE, [&](wxAuiNotebookEvent &e) {
...
void ScriptManagerWindow::showDocs(string url) {
    ...
    if (!found) {
        ...
        webview_docs_->Bind(
            wxEVT_WEBVIEW_NAVIGATING, [&](wxEvent &e) {
