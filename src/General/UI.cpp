...
void UI::setCursor(wxWindow *window, MouseCursor cursor) {
    switch (cursor) {
        ...
        case MouseCursor::Cross:
            window->SetCursor(wxCursor(wxCURSOR_CROSS));
            break;
        ...
        case MouseCursor::SizeNESW:
            window->SetCursor(wxCursor(wxCURSOR_SIZENESW));
            break;
        case MouseCursor::SizeNWSE:
            window->SetCursor(wxCursor(wxCURSOR_SIZENWSE));
            break;
        default:
...
int UI::px(Size size) {
    switch (size) {
        ...
        case Size::Pad:
            return px_pad_small;
        case Size::PadMinimum:
            return px_pad_min;
        case Size::Splitter:
            return px_splitter;
        case Size::SpinCtrlWidth:
            return px_spin_width;
        default:
            return 0;
    }
}
