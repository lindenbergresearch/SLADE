...
void MapEditContext::setEditMode(Mode mode) {
    ...
    switch (edit_mode_) {
        case Mode::Vertices:
            addEditorMessage("Vertices mode");
            break;
        case Mode::Lines:
            addEditorMessage("Lines mode");
            break;
        case Mode::Sectors:
            addEditorMessage("Sectors mode (Normal)");
            break;
        case Mode::Things:
            addEditorMessage("Things mode");
            break;
        case Mode::Visual:
            addEditorMessage("3d mode");
            break;
    };
...
void MapEditContext::cycleSectorEditMode() {
    switch (sector_mode_) {
        case SectorMode::Both:
            setSectorEditMode(SectorMode::Floor);
            break;
        case SectorMode::Floor:
...
void MapEditContext::updateInfoOverlay() {
    ...
    switch (edit_mode_) {
        case Mode::Vertices:
            info_vertex_.update(selection_.hilightedVertex());
            break;
        case Mode::Lines:
            info_line_.update(selection_.hilightedLine());
            break;
        case Mode::Sectors:
            info_sector_.update(selection_.hilightedSector());
            break;
        case Mode::Things:
            info_thing_.update(selection_.hilightedThing());
            break;
        default:
            break;
    }
...
void MapEditContext::drawInfoOverlay(const point2_t &size, float alpha) {
    switch (edit_mode_) {
        case Mode::Vertices:
            info_vertex_.draw(size.y, size.x, alpha);
            return;
        case Mode::Lines:
            info_line_.draw(size.y, size.x, alpha);
            return;
        case Mode::Sectors:
            info_sector_.draw(size.y, size.x, alpha);
            return;
        case Mode::Things:
            info_thing_.draw(size.y, size.x, alpha);
            return;
        case Mode::Visual:
            info_3d_.draw(size.y, size.x, size.x * 0.5, alpha);
            return;
    }
}
...
bool MapEditContext::handleAction(string id) {
    ...
    if (id == "mapw_mode_vertices") {
        ...
        switch (editMode()) {
            case Mode::Vertices:
                dlg.setType(MOBJ_VERTEX);
                break;
            case Mode::Lines:
                dlg.setType(MOBJ_LINE);
                break;
            case Mode::Sectors:
                dlg.setType(MOBJ_SECTOR);
                break;
            case Mode::Things:
                dlg.setType(MOBJ_THING);
                break;
            default:
                return true;
        }
