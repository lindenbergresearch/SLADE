...
bool ItemSelection::updateHilight(fpoint2_t mouse_pos, double dist_scale) {
    ...
    if (current != hilight_.index && selection_.empty()) {
        switch (context_->editMode()) {
            case Mode::Vertices:
                MapEditor::openObjectProperties(map.getVertex(hilight_.index));
                break;
            case Mode::Lines:
                MapEditor::openObjectProperties(map.getLine(hilight_.index));
                break;
            case Mode::Sectors:
                MapEditor::openObjectProperties(map.getSector(hilight_.index));
                break;
            case Mode::Things:
                MapEditor::openObjectProperties(map.getThing(hilight_.index));
                break;
            default:
                break;
        }
...
bool ItemSelection::selectWithin(const frect_t &rect, bool add) {
    ...
    switch (context_->editMode()) {
        case Mode::Vertices:
            selectVerticesWithin(context_->map(), rect);
            break;
        case Mode::Lines:
            selectLinesWithin(context_->map(), rect);
            break;
        case Mode::Sectors:
            selectSectorsWithin(context_->map(), rect);
            break;
        case Mode::Things:
            selectThingsWithin(context_->map(), rect);
            break;
        default:
            break;
    }
...
MapObject *ItemSelection::hilightedObject() const {
    ...
    switch (context_->editMode()) {
        case Mode::Vertices:
            return hilightedVertex();
        case Mode::Lines:
            return hilightedLine();
        case Mode::Sectors:
            return hilightedSector();
        case Mode::Things:
            return hilightedThing();
        default:
            return nullptr;
    }
}
