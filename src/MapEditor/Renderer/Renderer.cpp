...
void Renderer::drawMap2d() {
    ...
    if (mouse_state == Input::MouseState::Move) {
        ...
        switch (context_.editMode()) {
            case Mode::Vertices:
                renderer_2d_.renderMovingVertices(items, offset);
                break;
            case Mode::Lines:
                renderer_2d_.renderMovingLines(items, offset);
                break;
            case Mode::Sectors:
                renderer_2d_.renderMovingSectors(items, offset);
                break;
            case Mode::Things:
                renderer_2d_.renderMovingThings(items, offset);
                break;
            default:
                break;
        };
