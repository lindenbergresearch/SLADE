...
ArchiveManagerPanel::ArchiveManagerPanel(wxWindow *parent, STabCtrl *nb_archives)
    ...
    stc_tabs_->SetInitialSize(wxSize(UI::scalePx(250), -1));
...
void ArchiveManagerPanel::updateBookmarkListItem(int index) const {
    ...
    if (entry->isLocked())
        ...
        switch (entry->getState()) {
            case 0:
                list_bookmarks_->setItemStatus(index, LV_STATUS_NORMAL);
                break;
            case 1:
                list_bookmarks_->setItemStatus(index, LV_STATUS_MODIFIED);
                break;
            case 2:
                list_bookmarks_->setItemStatus(index, LV_STATUS_NEW);
                break;
            default:
                list_bookmarks_->setItemStatus(index, LV_STATUS_ERROR);
                break;
        }
}
        ...
        if (ep->isModified() && autosave_entry_changes > 0) {
            ...
            if (autosave_entry_changes > 1) {
                int result = wxMessageBox(
                    S_FMT("Save changes to entry \"%s\"?", ep->entry()->getName()),
                    "Unsaved Changes",
