...
enum {
    fir_min_width = (fir_width < 4 ? 4 : fir_width)
};
enum {
    fir_adj_width = fir_min_width / 4 * 4 + 2
};
enum {
    fir_stereo = 1
}; /* channel count, not boolean value */
enum {
    fir_write_offset = fir_adj_width * fir_stereo
};

enum {
    fir_buffer_size = fir_width * 2
};
...
static void gen_sinc(
    double rolloff, int width, double offset, double spacing, double scale,
    int count, short *out
) {
...
void fir_init() {
    ...
    for (i = 0; i < 32; i++) {
        ...
        for (n = res; --n >= 0;) {
            gen_sinc(
                rolloff, (int) (fir_adj_width * filter + 1) & ~1, pos, filter,
