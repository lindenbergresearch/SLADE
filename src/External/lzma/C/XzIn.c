...
UInt64 Xz_GetUnpackSize(const CXzStream *p) {
    UInt64 size = 0;
    size_t i;
    for (i = 0; i < p->numBlocks; i++) ADD_SIZE_CHECH(size, p->blocks[i].unpackSize);
    return size;
}
...
UInt64 Xz_GetPackSize(const CXzStream *p) {
    UInt64 size = 0;
    size_t i;
    for (i = 0; i < p->numBlocks; i++) ADD_SIZE_CHECH(size, (p->blocks[i].totalSize + 3) & ~(UInt64) 3);
    return size;
}
...
UInt64 Xzs_GetUnpackSize(const CXzs *p) {
    UInt64 size = 0;
    size_t i;
    for (i = 0; i < p->num; i++) ADD_SIZE_CHECH(size, Xz_GetUnpackSize(&p->streams[i]));
    return size;
}
