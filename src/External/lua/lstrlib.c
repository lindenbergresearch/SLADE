...
static int singlematch(
    MatchState *ms, const char *s, const char *p,
    const char *ep
) {
    if (s >= ms->src_end)
        return 0;
...
static void push_onecapture(
    MatchState *ms, int i, const char *s,
    const char *e
) {
    if (i >= ms->level) {
...
static void add_s(
    MatchState *ms, luaL_Buffer *b, const char *s,
    const char *e
) {
    size_t l, i;
    lua_State *L = ms->L;
...
static int getnumlimit(Header *h, const char **fmt, int df) {
    ...
    if (sz > MAXINTSIZE || sz <= 0)
        luaL_error(
            h->L, "integral size (%d) out of limits [1,%d]",
            sz, MAXINTSIZE
        );
    return sz;
}
...
** despite its size.
*/
static KOption getdetails(
    Header *h, size_t totalsize,
...
*/
static void copywithendian(
    volatile char *dest, volatile const char *src,
...
static int str_unpack(lua_State *L) {
    ...
    while (*fmt != '\0') {
        ...
        switch (opt) {
            case Kint:
            case Kuint: {
                lua_Integer res = unpackint(
                    L, data + pos, h.islittle, size,
                    (opt == Kint));
