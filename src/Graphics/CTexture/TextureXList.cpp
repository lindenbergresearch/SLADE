...
bool TextureXList::writeTEXTUREXData(ArchiveEntry *texturex, PatchTable &patch_table) {
    ...
    */
    ...
    switch (txformat) {
        case TXF_NORMAL:
            datasize = 4 + (26 * numtextures) + (10 * numpatchrefs);
            break;
        case TXF_NAMELESS:
        ...
        for (size_t k = 0; k < tex->nPatches(); ++k) {
            ...
            if (patch->getName().StartsWith("INVPATCH")) {
                ...
                pdef.patch = patch_table.patchIndex(
                    patch->getName());    // Note this will be -1 if the patch doesn't exist in the patch table. This should never happen with the texture editor, though.
