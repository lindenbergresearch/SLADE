...
bool SImage::rotate(int angle) {
    ...
    for (i = 0; i < numpixels; ++i) {
        switch (angle) {
            // Urgh maths...
            case 90:
                j = (((nh - 1) - (i % width)) * nw) + (i / width);
                break;
            case 180:
                j = (numpixels - 1) - i;
                break;
            case 270:
                j = ((i % width) * nw) + ((nw - 1) - (i / width));
                break;
            default:
                delete[] nd;
                if (mask) delete[] nm;
 ...
 * paletted
 *******************************************************************/
bool SImage::drawImage(SImage &img, int x_pos, int y_pos, si_drawprops_t &properties, Palette *pal_src, Palette *pal_dest) {
    // Check images
    if (!data || !img.data)
        return false;
...
bool SImage::adjust() {
    ...
    while (x2 > x1) {
        for (int i = 0; i < y2; ++i) {
            ...
            switch (type) {
                case PALMASK:
                    if (mask[p]) opaquefound = true;
                    break;
                case RGBA:
                    if (data[p * 4 + 3]) opaquefound = true;
                    break;
                case ALPHAMAP:
                    if (data[p]) opaquefound = true;
                    break;
            }
            if (opaquefound) break;
        }
