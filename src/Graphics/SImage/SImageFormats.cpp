...
bool SImage::loadBMF(const uint8_t *gfx_data, int size) {
    ...
    for (size_t i = 0; i < mf.pal_size; ++i) {
        palette.setColour(
            i + 1, rgba_t((ofs[(i * 3)] << 2) + (ofs[(i * 3)] >> 4),
...
bool SImage::loadJediFONT(const uint8_t *gfx_data, int size) {
    ...
    for (int i = 0; i < height; ++i) {
        for (int p = 0; p < width; ++p) {
            switch (bpc) {
                case 1:
                    mask[(i * width) + p] = ((gfx_data[o + i] >> (7 - p)) & 1) * 255;
                    break;
                case 2:
                    mask[(i * width) + p] = ((READ_B16(gfx_data, o + (i * 2)) >> (15 - p)) & 1) * 255;
                    break;
                case 3:
                    mask[(i * width) + p] = ((READ_B24(gfx_data, o + (i * 3)) >> (23 - p)) & 1) * 255;
                    break;
                case 4:
                    mask[(i * width) + p] = ((READ_B32(gfx_data, o + (i * 4)) >> (31 - p)) & 1) * 255;
                    break;
                default:
                    clearData();
